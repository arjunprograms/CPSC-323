...
keywords = ["integer", "if", "else", "fi", "while", "return", "get", "put", "function", "boolean", "true", "false"]
...
# Identifiers must start and end with a letter and can contain letters, digits, or underscores in between.
def get_identifier(source_code, position):
    ...
    # Ensure the first and last character are letters (per Rat24F specification)
    if not (lexeme[0].isalpha() and lexeme[-1].isalpha()):
        logging.error(f"Invalid identifier: {lexeme}")
...
if __name__ == "__main__":
    ...
    source_code = read_source_code('testcase1.txt')  # Read from test file
